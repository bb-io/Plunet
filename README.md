...
> Note: Plunet does not support removing a single callback listener. When a bird is deleted or suspended, Blackbird checks existing listeners, deletes all registered callbacks, and then re-subscribes the remaining listeners. If another system or cleanup process deletes callback listeners broadly, it may also remove Blackbird's listener and require the webhook subscription to be re-added by suspending and then publishing bird again.
